...
ToolBar {
  ...
  RowLayout {
    ...
    FileDialog {
        ...
        onAccepted: {
          TesseractVideoRecorder.OnSave(fileDialog.fileUrl)
          close()
        }
        onRejected: {
          TesseractVideoRecorder.OnCancel()
          close()
        }
    }
    ...
    Menu {
      ...
      MenuItem {
        text: "mp4"
        onTriggered: {
          TesseractVideoRecorder.OnStart("mp4")
          animation.start()
        }
      }
      MenuItem {
        text: "ogv"
        onTriggered: {
          TesseractVideoRecorder.OnStart("ogv")
          animation.start()
        }
      }
    }
