...
void KinematicGroupsModel::add(const QString& group_name, const QString& type, const QStringList& data)
{
  ...
  if (type == "Chain")
  {
    ...
    {
      this->thor_->getFwdKinematicsManager()->removeFwdKinematicSolver(group_name.toStdString());
      this->thor_->getInvKinematicsManager()->removeInvKinematicSolver(group_name.toStdString());
      ...
      if (!fwd_solver_names.empty())
      {
        ...
        auto fwd_kin = fwd_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), data[0].toStdString(), data[1].toStdString(), group_name.toStdString());
      ...
      if (!inv_solver_names.empty())
      {
        ...
        auto inv_kin = inv_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), data[0].toStdString(), data[1].toStdString(), group_name.toStdString());
      ...
      if (fwd_good || inv_good)
      {
        ...
        chain_groups[group_name.toStdString()] = {std::make_pair(data[0].toStdString(), data[1].toStdString())};
  ...
  else if (type == "Joint List")
  {
    ...
    if (!fwd_solver_names.empty() || !inv_solver_names.empty())
    {
      ...
      {
        auto fwd_factory = this->thor_->getFwdKinematicsManager()->getFwdKinematicFactory(fwd_solver_names[0]);
        auto fwd_kin = fwd_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), joints, group_name.toStdString());
        fwd_good = fwd_kin != nullptr && this->thor_->getFwdKinematicsManager()->addFwdKinematicSolver(fwd_kin);
      }
      if (!inv_solver_names.empty())
      {
        auto inv_factory = this->thor_->getInvKinematicsManager()->getInvKinematicFactory(inv_solver_names[0]);
        auto inv_kin = inv_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), joints, group_name.toStdString());
        inv_good = inv_kin != nullptr && this->thor_->getInvKinematicsManager()->addInvKinematicSolver(inv_kin);
      }
      ...
      if (fwd_good || inv_good)
      {
        ...
        joint_groups[group_name.toStdString()] = joints;
  ...
  else if (type == "Link List")
  {
    ...
    if (!fwd_solver_names.empty() || !inv_solver_names.empty())
    {
      ...
      {
        auto fwd_factory = this->thor_->getFwdKinematicsManager()->getFwdKinematicFactory(fwd_solver_names[0]);
        auto fwd_kin = nullptr; //fwd_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), joints, group_name.toStdString());
        fwd_good = fwd_kin != nullptr && this->thor_->getFwdKinematicsManager()->addFwdKinematicSolver(fwd_kin);
      }
      if (!inv_solver_names.empty())
      {
        auto inv_factory = this->thor_->getInvKinematicsManager()->getInvKinematicFactory(inv_solver_names[0]);
        auto inv_kin = nullptr; //inv_factory->create(this->thor_->getEnvironmentConst()->getSceneGraph(), joints, group_name.toStdString());
        inv_good = inv_kin != nullptr && this->thor_->getInvKinematicsManager()->addInvKinematicSolver(inv_kin);
      }
      ...
      if (fwd_good || inv_good)
      {
        ...
        link_groups[group_name.toStdString()] = links;
...
bool KinematicGroupsModel::removeRows(int row, int count, const QModelIndex &parent)
{
  ...
  QString group_name = row_item->data(this->NameRole).toString();
  QString type = row_item->data(this->TypeRole).toString();
  if (type == "Chain")
  {
